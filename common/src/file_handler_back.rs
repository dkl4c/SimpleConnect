use crate::{
    ...
    router::{self, MessageType, RouterInterface},
...
pub struct FileHandler {
    pub router: RouterInterface,
    pub file_workers: HashMap<String, FileRecvHandler>,
}

impl FileHandler {
    pub fn new(router: RouterInterface) -> Self {
        Self {
            router,
    ...
    pub fn run(mut self) {
        tokio::spawn(async move { self.message_recv().await });
    }
}

/// 处理所有文件、目录建立相关任务，在网络消息已被解析后
impl FileHandler {
    pub async fn message_recv(&mut self) -> Result<()> {
        while let Ok(msg) = self.router.recv().await {
            match msg.msg_type {
                router::MessageType::File => match msg.body {
                    router::MessageBody::String(_) => todo!(),
                    router::MessageBody::FileMetaData(file_data) => {
                        self.route_file_data(file_data).await?
                    }
                    _ => {}
                },
                router::MessageType::Directory => todo!(),
                router::MessageType::Message => todo!(),
                _ => {}
            }
        }
        Ok(())
    ...
    /// 任务开启入口
    /// 路由消息到对应的文件处理任务
    pub async fn route_file_data(&mut self, file_data: FileData) -> Result<()> {
        match file_data {
